...
int tui_region_offline(phys_addr_t *pa, unsigned long *size)
{
	...
	if (_svpregs[SSVP_TUI].state == SVP_STATE_ON) {
		...
		if (no_release_memory)
			...
			page = cma_alloc(cma, _svpregs[SSVP_TUI].count,
					SSVP_CMA_ALIGN_PAGE_ORDER);
...
int svp_region_offline(phys_addr_t *pa, unsigned long *size)
{
	...
	if (_svpregs[SSVP_SVP].state == SVP_STATE_ON) {
		...
		do {
			...
			page = cma_alloc(cma, _svpregs[SSVP_SVP].count,
					SSVP_CMA_ALIGN_PAGE_ORDER);
...
static int __init memory_ssvp_debug_init(void)
{
	...
	if (_SVP_MBSIZE > 0) {
		...
		if (no_release_memory) {
			wrap_svp_pages = cma_alloc(cma, _SVP_MBSIZE * SZ_1M >> PAGE_SHIFT,
					SSVP_CMA_ALIGN_PAGE_ORDER);
	...
	if (_TUI_MBSIZE > 0) {
		...
		if (no_release_memory) {
			wrap_tui_pages = cma_alloc(cma, _TUI_MBSIZE * SZ_1M >> PAGE_SHIFT,
					SSVP_CMA_ALIGN_PAGE_ORDER);
